...
class ChatMangeHelpUser extends StatelessWidget {
  final String chatId;
  final String userId;

  ChatMangeHelpUser({required this.chatId, required this.userId,});
             ...
             IconButton(
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  if (fileUrls != null && fileUrls!.isNotEmpty) {
                    if(Nomessages==true){
                    controller.ChatConnet(chatId.replaceAll(' ', ''),message,fileUrls!,currentUserData[0]['isAdmin']);}
                    else{
                      controller.ChatConnetFirestMessage(context, message, currentUserData[0]['name'], currentUserData[0]['uid'],fileUrls!,currentUserData[0]['isAdmin']);
                    }
                  } else if (message.isNotEmpty) {
                    if(Nomessages==true){
                    controller.ChatConnet(chatId.replaceAll(' ', ''),message,[],currentUserData[0]['isAdmin']);}
                    else{
                      controller.ChatConnetFirestMessage(context, message, currentUserData[0]['name'], currentUserData[0]['uid'],[],currentUserData[0]['isAdmin']);
                    }
                  }
                  fileUrls = null;
